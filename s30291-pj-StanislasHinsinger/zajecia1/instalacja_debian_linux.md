...
| Opcja |  Opis |
| --- | --- |
| RAM | Tutaj wprowadzamy ilość pamięci ram jaką chcemy przydzielić wirtualnemu systemowi. (Nie powinno się przydzielać wszystkich dostępnych zasobów hosta) [należy także dokonać rozsądnego przydziału zasobów, których rzeczywiście ewentualna maszyna może wykorzystać] <br>***Dla tego procesu instalacyjnego ustawimy: `2048 MB (zalecane na stronie dystrybutora jest 512MB, jednak ze względów na to, że system będzie miał zastosowanie serwerowe, a zatym serwował wielu użytkownikom i różnym aplikacjom w tym samym czasie, dodatkowa alokacja pamięci będzie w pełni zalecana)`.*** |
...
| Suwak Pamieci | Za pomocą tego suwaka możemy ustawić ilość przestrzeni dyskowej dostępnej dla danego systemu operacyjnego. <br>***Dla tego procesu instalacyjnego ustawimy: `40 GB`.*** (przestrzeń wystarczająca, jeżeli zamierzamy na takim serwerze udostepniać możliwość przechowywania plików przez użytkowników i ich udostępniania (FTP), a także jeżeli będzie potrzeba hostowania strony internetowej z bazą danych) |
| Type and Variant | Tutaj ustawiamy jakiego typu chcemy aby ten wirtualny dysk był. Opis każdego rodzaju znaleźć będzie można w komentarzach niżej. <br>***Dla tego procesu instalacyjnego wybieramy: `VDI (VirtualBox Disk Image)`*** (ponieważ będziemy mogli w łatwy sposób klonować obraz dysku (do backupów),  czy też bezproblemowo zwiększać jego pojemność względem zapotrzebownia)|
...
| Split into 2GB parts | Opcja ta pozwala na podzielenie dysku na 2GB części. Co mogłoby być wymagane w przypadku korzystania z systemu plików FAT16 (w którym maksymalna wielkość pliku to 2GB), lub tez chcielibyśmy w łatwiejszy sposób przesyłać mniejsze pliki dysku przez siec zamiast jednego dużego. <br>***Dla tego procesu instalacyjnego: `opcja ta ma być odznaczona`*** |
...
| VHD | Format dysku wirtualnego, który jest szeroko stosowany i obsługiwany przez różne platformy wirtualizacyjne, w tym przez VirtualBox. Utworzony przez Microsoft i wykorzysywany na Hyper-V oraz chmurze Microsoft Azure Public Cloud, |
...
| Opcja | Opis |
| --- | --- |
| Włącz szyfrowanie dysku | Opcja ta włącza szyfrowanie na dysku. <br>***W tym procesie instalacji: `opcja ta ma być odznaczona`.*** (jeżeli polityka prywatności i bezpieczeństwa danej firmy nie wymaga szyfrowania zawartości dysków to nie ma potrzeby aktywowania tej opcji dla tego wykorzystania serwerowego instalowanego systemu) |
...
| Włącz PAE/NX | Opcja ta pozwala na włączenie PAE/NX. <br>**PAE - Physical Address Extension** pozwala na korzystanie z więcej niż 4 GB RAM przy 32-bitowych systemach, jego aktywacja jest wymagana w przypadku przydzielenia większej ilości pamięci RAM do wirtualnej maszyny niż 4 GB. <br>**NX - No Execute** zabezpiecza system przed wykonaniem się nieautoryzowanego kodu umieszonego w obszarach pamięci (chroni przed `code injection`) <br>***W tym procesie instalacji: `opcja ta ma być odzaznaczona`.*** (zaznaczenie tej opcji będzie miało sens, tylko przy instalacji systemu 32-bitowego, gdyż wtedy pomimo ograniczeń i tak będzie on mógł alokować pamięć operacyjną na większej przestrzeni niż 32-bitowa wersja by pozwalała. Jeżeli hostujący system jest 64 bitowy i instalowany system jest 64 bitowy, to nie ma takiej potrzeby) |
...
### Pamięć
W tej sekcji można ustawiać kontrolery pamięci masowej i przydzielać do nich odpowiednie nośniki danych. Można także tworzyć emulowane nośniki zawierające dane z folderów na komputerze hosta. Dostępnych kontrolerów jest wiele, ale kluczowe to kontrolery IDE i SATA, które są domyślnie ustawione.

* *Dla tego procesu instalacyjnego w kontrolerze IDE powinien się znaleźć obraz instalatora `.iso`, a w kontrolerze SATA dysk `.vdi`, który został utworzony wraz z wirtualna maszyna.*
...
| Hot-pluggable | Opcja ta daje możliwość odłączania i ponownego podłączenia dysku podczas pracy systemu operacyjnego, bez potrzeby jego resetowania. |
| Użyj buforowania wejścia/wyjścia gospodarza | Opcja ta sprawia, że operacje wejścia i wyjścia są zapisywane w pamięci operacyjnej hosta przed zapisem na dysku, co może zwiększyć wydajność wykonywania operacji, ale zarazem zwiększyć zużycie pamięci operacyjnej hosta. |
...
| Opcja | Opis |
| --- | --- |
| Guided - use entire disk  | Automatyczne partycjonowanie całego dysku bez dodatkowych opcji, tworzy podstawowe partycje. |
| Guided - use entire disk and set up LVM  | Automatyczne partycjonowanie całego dysku z konfiguracją LVM (Logical Volume Manager), co umożliwia łatwiejsze zarządzanie partycjami i ich dynamiczne rozszerzanie. |
| Guided - use entire disk and set up encrypted LVM  | Automatyczne partycjonowanie całego dysku z konfiguracją zaszyfrowanego LVM, co zapewnia dodatkowe bezpieczeństwo poprzez szyfrowanie wszystkich danych na dysku. |
| Manual | 	Umożliwia ręczne partycjonowanie dysku, co daje pełną kontrolę nad podziałem na partycje i ich rozmiarem. |
...
| Configure Logical Volume Manager | Ta opcja umożliwia skonfigurowanie Logical Volume Manager (LVM), który dostarcza elastyczne zarządzanie przestrzenią dyskową poprzez tworzenie logicznych woluminów na jednym lub wielu fizycznych dyskach twardych. Dzięki LVM można łatwo zmieniać rozmiary partycji, dodawać nowe dyski do istniejących woluminów i wykonywać inne operacje zarządzania przestrzenią dyskową bez konieczności przerywania pracy systemu lub utraty danych. Konfiguracja LVM jest zalecana dla zaawansowanych użytkowników oraz w przypadku korzystania ze środowisk wymagających dynamicznego zarządzania przestrzenią dyskową. |
| Configute software RAID  | Opcja ta pozwala na konfiguracje software RAID (Redundant Array of Independent Disks). RAID jest technologią pozwalającą na łączenie wielu dysków twardych w jedną logiczną jednostkę, co zapewnia zwiększoną wydajność czy też niezawodność. W konfiguratorze można dodać lub usunąć konfiguracje RAID. Do wyboru będą konfiguracje  `RAID0`, `RAID1`, `RAID5`, `RAID6`, `RAID10`. Każda konfiguracja RAID sprawia, że zawartość dysków będzie czyszczona. |
| Configute encrypted volumes  | Opcja ta umożliwia skonfigurowanie zaszyfrowanych woluminów na dysku. Szyfrowanie zapewnia, że dane zapisane na dysku są chronione przed nieautoryzowanym dostępem. Podczas konfiguracji zostaniesz bedzie nalezalo wprowadzic hasło szyfrujące, które będzie wymagane przy każdym uruchomieniu systemu, aby uzyskać dostęp do zaszyfrowanych danych. W samej konfiguracji partycji będzie można wybrać różne metody szyfrowania danych w tym `AES`, `Blowfish`, `Serpent`, `Twofish`. Można wybrać także wielkość klucza (im dłuższy tym bardziej wymagający proces szyfrowania/odszyfrowywania) [128/192/256]. A także można wybrać algorytm ustalający wektor initializacji każdego sektora partycji, gdzie zalecane jest pozostawienie domyślnej opcji (`xts-plain64`), a ewentualny wybór innych powinien być dokonywany tylko ze względów na kompatybilność ze starszymi systemami. | 
| Configure iSCSI volumes | Opcja ta pozwala na konfigurację woluminów iSCSI (Internet Small Computer Systems Interface). iSCSI umożliwia połączenie i używanie zasobów dyskowych dostępnych przez sieć jako lokalnych dysków twardych. Konfiguracja obejmuje ustanowienie połączenia z serwerem iSCSI i konfigurację dostępnych woluminów do użycia podczas instalacji systemu. W konfiguratorze tym należy wprowadzić adres IP docelowego dysku sieciowego. |

| Konfiguracja RAID | Opis |
| ----------------- | ---- |
| RAID 0 (Stripe) | Dane są rozłożone na równych kawałkach na dwóch lub więcej dyskach, co zwiększa wydajność odczytu/zapisu. Brak redundancji danych. Awaria jednego dysku powoduje utratę wszystkich danych. |
| RAID 1 (Mirror) | Dane są duplikowane na dwóch lub więcej dyskach. Każda operacja zapisu jest replikowana na wszystkich dyskach w macierzy. Zapewnia wysoką niezawodność danych, ale kosztem połowy pojemności dysków. |
| RAID 5 | Dane i dane kontrolne są rozdzielane i zapisywane na wielu dyskach. Zapewnia wydajność i niezawodność danych, ale wymaga więcej zasobów na dane kontrolne. |
...
| Środowisko graficzne        | Wymagania sprzętowe    | Wydajność pracy    | Ergonomia    | Bezpieczeństwo    |
|----|---|---|---|---|
